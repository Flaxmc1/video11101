...
# ==================== DOCKER VPS CREATION - FIXED ====================
def create_docker_vps(name, hostname, cpu, ram, storage, os_type, owner_id, max_backups=3):
    """Create a new Docker VPS - FIXED VERSION with proper type conversion"""
    try:
        if not DOCKER_AVAILABLE:
            return None, "Docker not available"
        
        # FIX: Convert string values to integers FIRST
        cpu = int(cpu)  # Convert string to int
        ram = int(ram)  # Convert string to int
        storage = int(storage)  # Convert string to int
        max_backups = int(max_backups)  # Convert string to int
        
        # Validate ranges
        if cpu < 1 or cpu > 16:
            return None, "CPU must be between 1 and 16"
        if ram < 1 or ram > 32:
            return None, "RAM must be between 1 and 32 GB"
        if storage < 10 or storage > 1000:
            return None, "Storage must be between 10 and 1000 GB"
        ...
        # FIX: Use converted integers for calculations
        cpu_quota = cpu * 100000  # Now safe because cpu is int
        mem_limit = f"{ram}g"  # ram is int, convert to string with 'g'
...
def setup_ssh(container, password):
    ...
    try:
        commands = [
            ...
            ['bash', '-c', "sed -i 's/#PasswordAuthentication yes/PasswordAuthentication yes/' /etc/ssh/sshd_config 2>/dev/null || echo 'PasswordAuthentication yes' >> /etc/ssh/sshd_config"],
...
<body>
    ...
    <div class="sidebar">
        ...
        <div style="flex: 1;">
            ...
            <a href="#" class="nav-item" onclick="showCreateVPS()">
                <span class="icon">➕</span> <span>Create VPS</span>
            </a>
        </div>
    ...
    <div class="modal" id="createVPSModal">
        <div class="modal-content">
            ...
            <form method="POST" action="/create_vps" onsubmit="return validateForm()">
                    ...
                    <div id="backupOptions" style="display: none;">
                        <div class="form-group">
                            ...
                            <select name="max_backups">
                                <option value="0">No backups</option>
    ...
    <script>
        ...
        function validateForm() {
            ...
            if (cpu < 1 || cpu > 16) {
                alert('CPU must be between 1 and 16');
                return false;
            }
            if (ram < 1 || ram > 32) {
                alert('RAM must be between 1 and 32 GB');
                return false;
            }
            if (storage < 10 || storage > 500) {
                alert('Storage must be between 10 and 500 GB');
                return false;
            }
            return true;
        }
...
# FIXED: create_vps route with proper type conversion
...
def create_vps():
    try:
        ...
        cpu_str = request.form['cpu']
        ram_str = request.form['ram']
        storage_str = request.form['storage']
        os_type = request.form['os']
        max_backups_str = request.form.get('max_backups', '3')
        
        # FIX: Convert to int with validation
        try:
            cpu = int(cpu_str)
            ram = int(ram_str)
        ...
        # Validate ranges
        if cpu < 1 or cpu > 16:
            flash('CPU must be between 1 and 16', 'error')
            return redirect(url_for('dashboard'))
        
        if ram < 1 or ram > 32:
            flash('RAM must be between 1 and 32 GB', 'error')
            return redirect(url_for('dashboard'))
        
        if storage < 10 or storage > 500:
            flash('Storage must be between 10 and 500 GB', 'error')
            return redirect(url_for('dashboard'))
        
        result, error = create_docker_vps(name, hostname, cpu, ram, storage, os_type, owner_id, max_backups)
        ...
        c.execute('''INSERT INTO vps_instances 
                  ...
                  (name, owner_id, hostname, '127.0.0.1', result['ssh_port'], result['ssh_password'], 
                   cpu, ram, storage, os_type, 'running', result['container_id'], max_backups, datetime.now()))
        vps_id = c.lastrowid
        conn.commit()
        conn.close()
        
        log_activity(session['user_id'], 'create_vps', f'Created VPS {name} for user {owner_id}')
        flash(f'✅ VPS created successfully! SSH Port: {result["ssh_port"]}', 'success')
...
if __name__ == '__main__':
    ...
    print("   • ✅ Multi-OS support (12+ distributions)")
    print("   • ✅ User management")
    print("   • ✅ Activity logging")
    print("   • ✅ Bug-free type conversion")
    print("   • ✅ Pterodactyl-style interface")
